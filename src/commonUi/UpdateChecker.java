...
public class UpdateChecker implements Runnable
{
	...
	public void run()
	{
		String availableRelease = this.getReleaseFromServer(Constants.FILE_NAME_RELEASE);
		if (availableRelease == null)
		{
			if (!this.background)
			{
				DialogWindow.showError(
						(Frame)parent, 
						SternResources.UpdateServerCannotBeReached(false),
        ...
        String recommendedRelease = this.getReleaseFromServer(Constants.FILE_NAME_RELEASE_RECOMMENDED);
		...
		if (currentReleaseLong < availableReleaseLong)
		{
			...
			if (currentReleaseLong >= recommendedReleaseLong)
			{
				...
				if (this.lastReleaseFoundLong < availableReleaseLong)
				{
					...
					DialogWindow.showInformation(
							...
							new MessageWithLink(
									(Frame)parent,
									SternResources.UpdateAvailable(false) + " - <a href=\""+urlString+"\">"+urlString+"</a>"),
							SternResources.Update(false));
				}
			}
			else
			{
				...
				DialogWindow.showInformation(
						...
						new MessageWithLink(
								(Frame)parent,
								SternResources.UpdateAvailableImportant(false) + " - <a href=\""+urlString+"\">"+urlString+"</a>"),
						SternResources.Update(false));
			}
		}	
		...
		if (!this.background && aktuell)
		{
			DialogWindow.showInformation(
					(Frame)parent, 
					SternResources.UpdateUpToDate(false),
					SternResources.Update(false));
		}
	}
